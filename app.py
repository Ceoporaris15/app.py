...
st.markdown("""
    ...
    /* HUDレイアウト: ラベルとバーを横並びに */
    .status-row {
        display: flex; align-items: center; margin-bottom: 4px;
    }
    .status-label {
        width: 65px; font-size: 0.65rem; color: #aaa; font-weight: bold;
    }
    ...
    .live-log {
        ...
        font-size: 0.7rem; color: #00ffcc; height: 60px; overflow-y: auto;
    }
    .self-hud {
    ...
    .victory-screen { text-align: center; color: #d4af37; font-size: 2rem; margin-top: 20%; border: 2px solid #d4af37; padding: 20px; }
    .defeat-screen { text-align: center; color: #ff4b4b; font-size: 2rem; margin-top: 20%; border: 2px solid #ff4b4b; padding: 20px; }
...
if not st.session_state.room_id:
    ...
    if st.button("DEPLOY"):
        if role == "p1":
            init_data = {"id": rid, "p1_hp": 1000.0, "p2_hp": 1000.0, "p1_colony": 50.0, "p2_colony": 50.0, "p1_nuke": 0.0, "p2_nuke": 0.0, "turn": "p1", "ap": 2, "chat": ["🛰️ 通信確立。"]}
    ...
    if data[f"{me}_colony"] <= 0 or data[f"{me}_hp"] <= 0:
        ...
        if st.button("EXIT"): st.session_state.room_id = None; st.rerun()
        st.stop()
    if data[f"{opp}_colony"] <= 0 or data[f"{opp}_hp"] <= 0:
        st.markdown(f'<div class="victory-screen">勝利<br>{enemy_name} 征服</div>', unsafe_allow_html=True)
        if st.button("EXIT"): st.session_state.room_id = None; st.rerun()
        st.stop()
    ...
    st.markdown(f"""
    <div class="self-hud">
        <div style="font-size:1rem; color:#d4af37; font-weight:bold; margin-bottom:8px;">{my_name} <span style="font-size:0.6rem; color:#ff4b4b;">(ATK: {current_atk:.0f})</span></div>
    ...
    if data['turn'] == me:
        if my_nuke >= 200:
            if st.button("🚨 核兵器発射 (NUKE)", type="primary", use_container_width=True):
                sync(st.session_state.room_id, {f"{opp}_hp": data[f"{opp}_hp"]*0.2, f"{opp}_colony": data[f"{opp}_colony"]*0.2, f"{me}_nuke": 0, "ap": 0, "chat": data['chat']+[f"☢️ {my_name}が核を使用！"]})
                st.rerun()
        ...
        if c1.button("🛠️\n軍拡", **conf):
            ...
            st.rerun()
        if c2.button("🛡️\n防衛", **conf):
            sync(st.session_state.room_id, {f"{me}_colony": data[f"{me}_colony"]+35, "ap": data['ap']-1, "chat": data['chat']+[f"🛡️ {my_name}: 防衛。"]})
            st.rerun()
        if c3.button("🕵️\n工作", **conf):
            success = random.random() < 0.5
            sync(st.session_state.room_id, {f"{opp}_nuke": max(0, data[f"{opp}_nuke"] - (100 if success else 0)), "ap": data['ap']-1, "chat": data['chat']+[f"🕵️ {my_name}: 工作{'成功' if success else '失敗'}。"]})
            st.rerun()
        if c4.button("⚔️\n進軍", **conf):
        ...
        if c5.button("🚩\n占領", **conf):
            ...
            sync(st.session_state.room_id, {f"{me}_colony": data[f"{me}_colony"]+55, f"{me}_nuke": max(0, my_nuke - (30 if rebel else 0)), "ap": data['ap']-1, "chat": data['chat']+[f"🚩 {my_name}: {'反乱' if rebel else '占領'}。"]})
            st.rerun()
