...
if st.session_state.briefing:
    ...
    <div class="brief-container">
        <div class="brief-h1">【 戦略指令：防衛システム改修 】</div>
        
        <div class="brief-section">
            <b style="color:#ff4b4b;">🛡️ 防衛プロトコル</b><br>
            実行時に<b>植民地を20消費</b>します。その後、独立した2種類の抽選が行われます。<br><br>
            ・<b>通常シールド</b>：進軍を1度防ぐ <span class="prob-tag">1/4 (25%)</span><br>
            ・<b>対核シールド</b>：核兵器を1度防ぐ <span class="prob-tag">1/15 (約6.6%)</span><br>
            <small>※シールドはそれぞれ個別に蓄積（スタック）されます。</small>
        </div>

        <div class="brief-section">
            <b>⚔️ 進軍</b><br>
            敵の「通常シールド」がある場合、1つ破壊して無効化されます。盾がない場合はダメージを与えます。
        </div>

        <div class="brief-section">
            <b>☢️ 核兵器（NUKE）</b><br>
            敵の「対核シールド」がある場合、1つ破壊して無効化されます。盾がない場合は壊滅的打撃を与えます。
        </div>
    </div>
    """, unsafe_allow_html=True)
    if st.button("新規定を承認し、戦地へ向かう"):
    ...
    if st.button("接続"):
        if role == "p1":
            init_data = {
                ...
                "p1_nuke": 0.0, "p2_nuke": 0.0, "turn": "p1", "ap": 2, "chat": ["🛰️ 防衛システム分離完了。"],
...
# --- 5. ゲーム本編 ---
    ...
    if my_colony <= 0 or data[f"{me}_hp"] <= 0:
        st.error(f"【 敗北 】 {my_name}は陥落。"); st.stop()
    ...
    s_normal = data.get(f'{me}_shield', 0)
    s_nuke = data.get(f'{me}_nuke_shield_count', 0)
    ...
    <div style="background:#050505; border:1px solid #00ffcc; padding:12px; border-radius:5px; margin-bottom:15px;">
        <div style="font-weight:bold; margin-bottom:8px;">{my_name} 
            <span style="color:#3498db; font-size:0.75rem;"> [通常盾:{s_normal}]</span> 
            <span style="color:#9b59b6; font-size:0.75rem;"> [対核盾:{s_nuke}]</span>
        </div>
    ...
    if data['turn'] == me:
        ...
        if c2.button("🛡️防衛"):
            if my_colony < 20:
                ...
                # 1/4(25%)で通常盾+1、1/15(約6.6%)で対核盾+1
                add_normal = 1 if random.random() < 0.25 else 0
                add_nuke = 1 if random.random() < (1/15) else 0
                
                msg = f"🛡️ {my_name}: 防衛(-20)"
                if add_normal: msg += " [通常盾+1]"
                if add_nuke: msg += " [対核盾+1]"
                
                sync(st.session_state.room_id, {
                    f"{me}_colony": new_colony,
                    f"{me}_shield": data[f"{me}_shield"] + add_normal,
                    f"{me}_nuke_shield_count": data[f"{me}_nuke_shield_count"] + add_nuke,
                    "ap": data['ap']-1,
                    "chat": data['chat'] + [msg]
                })
                st.rerun()
        ...
        if c3.button("🕵️工作"):
            ...
            sync(st.session_state.room_id, {**up, "chat": data['chat']+[f"🕵️ {my_name}: 工作"]})
            st.rerun()

        if c4.button("⚔️進軍"):
            if data[f"{opp}_shield"] > 0:
                sync(st.session_state.room_id, {f"{opp}_shield": data[f"{opp}_shield"]-1, "ap": data['ap']-1, "chat": data['chat']+[f"⚔️ {enemy_name}が通常盾を使用"]})
            else:
        ...
        if c5.button("🚩占領"):
            ...
            sync(st.session_state.room_id, {f"{me}_colony": my_colony+55, f"{me}_nuke": max(0, my_nuke - (30 if rebel else 0)), "ap": data['ap']-1, "chat": data['chat']+[f"🚩 {my_name}: 占領"]})
            st.rerun()
        ...
        if my_nuke >= 200:
            if st.button("🚨 核兵器投下", type="primary"):
                if data[f"{opp}_nuke_shield_count"] > 0:
                    sync(st.session_state.room_id, {f"{opp}_nuke_shield_count": data[f"{opp}_nuke_shield_count"]-1, f"{me}_nuke": 0, "ap": 0, "chat": data['chat']+[f"☢️ {enemy_name}が対核盾で迎撃！"]})
                else:
                    sync(st.session_state.room_id, {f"{opp}_hp": data[f"{opp}_hp"]*0.2, f"{opp}_colony": data[f"{opp}_colony"]*0.2, f"{me}_nuke": 0, "ap": 0, "chat": data['chat']+[f"☢️ {my_name}の核が直撃！"]})
                st.rerun()
